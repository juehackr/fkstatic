 ...
 * @copyright (c) 2010 Simon Tang (http://nanjiangxian.com/)
 */
(function(jq) {
    jQuery.fn.imageMaps = function(setting) {
		...
		container.each(function(){
			...
			images.each(function(){
				var _img_conrainer = jq(this).parent();
				_img_conrainer.append('<div class="button-conrainer"><input type="button" value="增加连接" /></div>').append('<div class="link-conrainer"></div>').append(jq.browser.msie ? jq('<div class="position-conrainer" style="position:absolute"></div>').css({
					background:'#fff',
					opacity:0
				}) : '<div class="position-conrainer" style="position:absolute"></div>');
				var _img_offset = jq(this).offset();
				...
				_img_conrainer.find('.position-conrainer').css({
					...
					left: _img_offset.left - _img_conrainer_offset.left + a,
					width:jq(this).width(),
					height:jq(this).height(),
					border:'dotted 1px red',
					margin:'-1px'
				});
				var map_name = jq(this).attr('usemap').replace('#','');
				if(map_name !=''){
					var index = 1;
					var _link_conrainer = _img_conrainer.find('.link-conrainer');
					var _position_conrainer = _img_conrainer.find('.position-conrainer');
					var image_param = jq(this).attr('name') == '' ? '' : '['+ jq(this).attr('name') + ']';
					container.find('map[name='+map_name+']').find('area[shape=rect]').each(function(){
						var coords = jq(this).attr('coords');
						_link_conrainer.append('<p ref="'+index+'" class="map-link"><span class="link-number-text">连接 '+index+'</span>: <input id="link'+index+'" type="text" size="60" name="link'+image_param+'['+index+']" value="'+jq(this).attr('href')+'" /><input type="hidden" id="map'+index+'" class="rect-value" name="rect'+image_param+'['+index+']" value="'+coords+'" /></p>');
						coords = coords.split(',');
						_position_conrainer.append('<div ref="'+index+'" class="map-position" style="left:'+coords[0]+'px;top:'+coords[1]+'px;width:'+(coords[2]-coords[0])+'px;height:'+(coords[3]-coords[1])+'px;"><div class="map-position-bg"></div><span class="link-number-text">连接 '+index+'</span><span class="delete">X</span><span class="resize"></span></div>');
						index++;
					});
				}
			});
		...
		jq('.button-conrainer input[type=button]').click(function(){
			var _link_conrainer = jq(this).parent().parent().find('.link-conrainer');
			var _position_conrainer = jq(this).parent().parent().find('.position-conrainer');
			var index = _link_conrainer.find('.map-link').length +1;
			var image = jq(this).parent().parent().find('img[ref=imageMaps]').attr('name');
			image = (image == '' ? '' : '['+ image + ']');
			_link_conrainer.append('<p ref="'+index+'" class="map-link"><span class="link-number-text">连接 '+index+'</span>: <input id="link'+index+'" type="text" size="60" name="link'+image+'[]" value="" /><input id="map'+index+'"  type="hidden" class="rect-value" name="rect'+image+'[]" value="10,10,100,40" /></p>');
			_position_conrainer.append('<div ref="'+index+'" class="map-position" style="left:500px;top:10px;width:90px;height:30px;"><div class="map-position-bg"></div><span class="link-number-text">连接 '+index+'</span><span class="delete">X</span><span class="resize"></span></div>');
			bind_map_event();
			define_css();
		});
		...
		function bind_map_event(){
			jq('.position-conrainer .map-position .map-position-bg').each(function(){
				var map_position_bg = jq(this);
				var conrainer = jq(this).parent().parent();
				...
				conrainer.mousemove(function(event){
					...
					jq('.link-conrainer p[ref='+map_position.attr('ref')+'] .rect-value').val(new Array(left,top,right,bottom).join(','));
					return false;
			...
			jq('.position-conrainer .map-position .resize').each(function(){
				var map_position_resize = jq(this);
				var conrainer = jq(this).parent().parent();
			...
			jq('.position-conrainer .map-position .delete').unbind('click').click(function(){
				var ref = jq(this).parent().attr('ref');
				var _link_conrainer = jq(this).parent().parent().parent().find('.link-conrainer');
				var _position_conrainer = jq(this).parent().parent().parent().find('.position-conrainer');
				...
				_link_conrainer.find('.map-link').each(function(){
					jq(this).attr('ref',index).find('.link-number-text').html('Link '+index);
					index ++;
				});
				index = 1;
