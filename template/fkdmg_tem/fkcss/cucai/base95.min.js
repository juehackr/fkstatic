...
function setCookie(k, v, d, s) {
	var o = new Date;
	o.setTime(o.getTime() + 864e5 * d + 1000 * (s || 0)), document.cookie = k + "=" + v + ";path=/;expires=" + o.toGMTString()
}
window.linkify = (function () {
	var f = "[a-z\\d.-]+://",
		h = "(?:(?:[0-9]|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])\\.){3}(?:[0-9]|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])",
		g = "(?:(?:[^\\s!@#$%^&*()_=+[\\]{}\\\\|;:'\",.<>/?]+)\\.)+",
		m = "(?:at|biz|com|ch|co|de|edu|es|eu|fr|gov|info|it|ly|me|mobi|nl|net|org|to|uk|us|ws)",
		c = "(?:" + g + m + "|" + h + ")",
		n = "(?:[;/][^#?<>\\s]*)?",
		d = "(?:\\?[^#<>\\s]*)?(?:#[^<>\\s]*)?",
		e = "\\b" + f + "[^<>\\s]+",
		b = "\\b" + c + n + d + "(?!\\w)",
		l = "mailto:",
		i = "(?:" + l + ")?[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@" + c + d + "(?!\\w)",
		a = new RegExp("(?:" + e + "|" + b + "|" + i + ")", "ig"),
		j = new RegExp("^" + f, "i"),
		k = {
			"'": "`",
			">": "<",
	...
	return function (u, D) {
		...
		while (w = a.exec(u)) {
			...
			do {
				...
				z = k[x];
				if (z) {
					q = y.match(new RegExp("\\" + z + "(?!$)", "g"));
					A = y.match(new RegExp("\\" + x, "g"));
					if ((q ? q.length : 0) < (A ? A.length : 0)) {
						y = y.substr(0, y.length - 1);
						C--
					}
				}
				y = y.replace(/(?:[!?.,:;'"]|(?:&|&amp;)(?:lt|gt|quot|apos|raquo|laquo|rsaquo|lsaquo);)$/, function (E) {
					C -= E.length;
					return ""
				})
			} while (y.length && y !== o);
			p = y;
			if (!j.test(p)) {
				p = (p.indexOf("@") !== -1 ? (!p.indexOf(l) ? "" : l) : !p.indexOf("irc.") ? "irc://" : !p.indexOf("ftp.") ? "ftp://" : "http://") + p
			}
			if (r != B) {
				s.push([u.slice(r, B)]);
				r = C
		...
		for (t = 0; t < s.length; t++) {
			text = s[t][0];
			p = s[t][1];
			target = text.indexOf(window.location.hostname) >= 0 ? "" : 'target="_blank" ';
			v += (p ? "<a " + target + 'href="' + p + '">' + text + "</a>" : text)
		}
		return v || u
	}
})();
if (getCookie('is_human')) {
	...
	$(document).one('keydown', 'input[type=text]', function () {
		setCookie('is_human', 1, 8640000);
		is_human = true;
	});
...
$.ajaxSetup({
	...
	error: function () {
		if (ajax_anim) {
			loadingBar.css('width', '102%').addClass('error').fadeOut(2000, function () {
				loadingBar.removeClass('error').hide();
			});
		}
	},
	complete: function (xhr, status) {
		if (status != 'error') loadingBar.hide();
		ajax_anim = false;
	},
	xhr: function () {
...
function resized(autosized) {
	...
	if (!autosized) {
		$('.autosize').not('.autosized').addClass('autosized').autosize({
			placeholder: 0,
			callback: function () {
				setTimeout(function () {
					resized(true);
		...
		if (is_ie || is_safari) {
			$('[placeholder]').not('.polyfilled').addClass('polyfilled').focus(function () {
				var i = $(this);
	...
	if (that.data('confirm') == null || confirm(that.data('confirm')))
		$.get(that.data('href') || this.href, function (re) {
			if (re.substring(0, 7) == 'script:') eval(re.substring(7));
				...
				$.colorbox({
					html: re,
					overlayClose: that.data('no-overlay-close') == null,
					height: 'auto',
					onComplete: function () {
						setTimeout(resized, 100);
						$('#cboxContent h6').prepend('<a onclick="$.colorbox.close();">×</a>');
					}
				});
			}
		});
...
$(document).click(function (e) {
	if (dd_box && !dd_box.is(e.target) && !dd_box.has(e.target).length) {
		dd_box.hide();
		dd_box = false;
	}
});
$('.toggle_mobile_search').on('click touchstart', function () {
	$('body').toggleClass('show_mobile_search');
	setTimeout(function () {
...
function show_support_overlay() {
	var el = $('#support_overlay'),
		img = $('#media_container > img');
	if (el.length && img.width() > 370 && img.height() > 180) {
		...
		if (img.height() > 470) {
			(function (d, s, id) {
				...
				js.id = id;
				js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.5&appId=114593902037957";
...
$(function () {
	...
	$('.media_search').submit(function () {
		...
		if (q) {
			...
			setCookie('manual_search', 1, 0, 2);
		}
		if (!$(this).find('[name="hp"]').val()) {
			$(this).find('[name="hp"]').removeAttr('name');
		}
		if (!$(this).find('[name="image_type"]').val() || $(this).find('[name="image_type"]').val() == 'video') {
			$(this).find('[name="image_type"]').removeAttr('name');
		}
		if (!$(this).find('[name="order"]').val()) {
			$(this).find('[name="order"]').removeAttr('name');
		}
		if (!$(this).find('[name="cat"]').val()) {
			$(this).find('[name="cat"]').removeAttr('name');
		}
		if (!$(this).find('[name="min_width"]').val()) {
			$(this).find('[name="min_width"]').removeAttr('name');
		}
		if (!$(this).find('[name="min_height"]').val()) {
			$(this).find('[name="min_height"]').removeAttr('name');
		}
	});
	...
	setTimeout(function () {
		if (!('ontouchstart' in window || window.location.hash || $(document).scrollTop())) $('[data-autofocus]').last().focus();
	}, 250);
	if (~$.inArray(LANG, ['de', 'en', 'es', 'fr', 'it', 'ja', 'ko', 'pt']))
		$('.media_search input[name="q"]').autoComplete({
			onSelect: function (e) {
				if (e.type != 'keydown') $('.media_search').submit();
			},
	...
	if ($('.media_list').length) {
		...
		if ($('.flex_grid.video').length) {
			flexVideoGrid($('.flex_grid.video.px_content'), 280, 0, false, '');
			$('.flex_grid.video.sponsored').each(function () {
				flexVideoGrid($(this), parseInt($(this).data('rowHeight')) || 280, parseInt($(this).data('rows')) || 1, false);
			});
		} else {
			$('.flex_grid.credits').flexImages({
				rowHeight: 300
			});
			$('.flex_grid.sponsored').each(function () {
				$(this).flexImages({
		...
		$('#media_container img').click(function (e) {
			...
			if (!container.data('cloned')) {
				container.data('cloned', 1).find('#download_menu_clone').append($('.download_menu').clone(true, true)).end().find('#like_buttons_clone').append($('.like_buttons').clone(true, true));
			}
			$('#media_show .overlay').fadeIn(100);
		});
		$('#media_show .overlay').click(function (e) {
			if ($(e.target).is('#support_overlay .pure-button, #download_menu_clone *:not(.modal)')) {
				e.stopPropagation();
				return;
			} else if ($(e.target).is('#like_buttons_clone *')) return;
			var support_overlay_click = $('#support_overlay').is(':visible');
			$(this).fadeOut(200, function () {
				if (support_overlay_click) $('#support_overlay').remove();
			});
		});
		...
		flexVideoGrid($('.flex_grid.video.px_content'), 240, 4, true);
		flexVideoGrid($('.flex_grid.video.sponsored'), 240, 2, true);
	...
	if ($('#media_show').length) {
		...
		$('#media_show .init').mouseleave(function () {
		...
		$(document).click(function (e) {
			if (dl_menu && !dl_menu.is(e.target) && !dl_menu.has(e.target).length) {
				dl_menu.hide();
				dl_menu = null;
			}
		});
	...
	if (window.location.host.indexOf('safesearch') > -1 || g_rated == 'permanent') {
		g_rated = true;
		$('.toggle_g_rated').attr('disabled', true).parent().parent().attr('title', 'SafeSearch is enabled and locked.').click(function () {
			alert(this.title);
		});
	} else
		$(document).on('click', '.toggle_g_rated', function () {
			g_rated = $(this).is(':checked');
			setCookie('g_rated', g_rated ? 1 : '', 3650);
	...
	if (g_rated) {
		...
		$('.flex_grid .item.nsfw').each(function () {
	...
	if (location.pathname.length > 4 && mobile_os && !getCookie('seen_app_hint') && mobile_os != 'Windows') {
		...
		if (mobile_os == 'iOS') html += '<a href="//itunes.apple.com/app/id1178021455"><img style="vertical-align:middle" src="//fkstatic.fkdmg.com/static/img/app_badge_apple.png" height="36"></a>';
		else html += '<a href="//play.google.com/store/apps/details?id=com.pixabay.pixabayapp"><img style="vertical-align:middle" src="//fkstatic.fkdmg.com/static/img/app_badge_google.png" height="36"></a>';
		$('body').append(html + '</div>');
		$('#app_hint a, #app_hint .times').click(function () {
			setCookie('seen_app_hint', 1, 14);
			$('#app_hint').fadeOut();
		});
	}
	$('a.ga_track, .ga_track a').click(function () {
		var url = this.href,
			target = this.target;
		if (typeof ga) ga('send', 'event', 'outbound', 'click', url, {
			'transport': 'beacon',
			'hitCallback': function () {
				if (!target) document.location = url;
			}
		});
	});
});
